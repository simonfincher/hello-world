...
tbl_engine
    Specify table engine to use (default MyISAM)
EOF
...
my ($db_name, $tbl_name, $tbl_engine) = ("test", "t", "MyISAM");
$db_name = shift (@ARGV) if @ARGV == 3;
$tbl_name = shift (@ARGV) if @ARGV == 2;
$tbl_engine = shift (@ARGV) if @ARGV == 1;

$tbl_engine = "BerkeleyDB" if $tbl_engine =~ /^BDB$/i;
$tbl_engine =~ /^[a-z0-9_]+$/i    # letters, digits, underscore allowed
...
print "Server version: $ver_str\n" if defined ($ver_str);
print "Using database: $db_name\n";
print "Using table: $tbl_name\n";
print "Using table engine: $tbl_engine\n";

if ($ver_num < 32300 && uc ($tbl_engine) ne "ISAM")
{
  die "ISAM is the only table engine available prior to MySQL 3.23.0\n";
}
...
$seq = get_seq_val ($dbh);
$dbh->do ("UPDATE $tbl_name SET seq = NULL where seq = ?", undef, $seq);
$seq2 = $dbh->selectrow_array ("SELECT MAX(seq) FROM $tbl_name");
print "  Sequence value before/after updating to NULL: $seq/$seq2\n";
...
# If server is older than 3.23.0, don't add the TYPE clause, it will
# cause a syntax error.  (Table engine had better be ISAM.)
...
my ($major, $minor, $teeny);
  ...
  $ver_str = $dbh->{mysql_serverinfo};
  # use old form if new form unavailable
  $ver_str = $dbh->{serverinfo} unless defined ($ver_str);
