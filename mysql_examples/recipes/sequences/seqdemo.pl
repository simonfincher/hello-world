...
# value (LAST_INSERT_ID() function), and then with client-side value
...
# Generate a number, and then show that issuing another query may
...
# Run a query, and then compare the value returned by LAST_INSERT_ID()
