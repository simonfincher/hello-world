...
my $stmt = "SELECT * FROM $db_name.$tbl_name";
$stmt .= " ORDER BY " . $dbh->quote_identifier ($sort_col);
$stmt .= " LIMIT 50";
...
print p ("Click on a column name to sort by that column.");
