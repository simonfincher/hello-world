...
print "Execute INSERT statement using prepare() + bind_param() + execute()\n";
#@ _PLACEHOLDER_BIND_PARAM_1_
my $sth = $dbh->prepare ("INSERT INTO profile (name,birth,color,foods,cats)
                          VALUES(?,?,?,?,?)");
...
print "Execute SELECT statement using prepare() + bind_param() + execute()\n";
#@ _PLACEHOLDER_BIND_PARAM_2_
my $sth = $dbh->prepare ("SELECT * FROM profile WHERE cats > ?");
$sth->bind_param (1, 2);
$sth->execute ();
while (my $ref = $sth->fetchrow_hashref ())
