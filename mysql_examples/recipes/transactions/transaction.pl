...
# First example: transaction handling with everything written out
...
# Second example: transaction handling using convenience functions
...
# Third and fourth examples: like first and second, but the transaction
# fails so that rollback can be demonstrated
