...
def make_definition_list (terms, definitions, encode = True):
  ...
  for i in range (len (terms)):
    ...
    if definition is None:
      definition = ""
    # make sure term/definition are strings, and then encode if necessary
    term = str (term)
    definition = str (definition)
    if encode:
      term = cgi.escape (term, 1)
      definition = cgi.escape (definition, 1)
    list = list + "<dt>" + term + "</dt>\n"
    list = list + "<dd>" + definition + "</dd>\n"
  list = list + "</dd>\n"
  return list
#@ _MAKE_DEFINITION_LIST_
    ...
    result_str = result_str + \
          "<input type=\"radio\" name=\"%s\" value=\"%s\"%s />%s" \
            % (cgi.escape (name, 1),
              ...
              cgi.escape (label, 1))
    if vertical:
      result_str = result_str + "<br />"  # display items vertically
    result_str = result_str + "\n"
  return result_str
#@ _MAKE_RADIO_GROUP_
...
def make_scrolling_list (name, values, labels, default, size, multiple):
  ...
  if multiple:
    ...
    multiple = ""
  result_str = "<select name=\"%s\" size=\"%s\"%s>\n%s</select>\n" \
        % (cgi.escape (name, 1),
          cgi.escape (size, 1),
          multiple,
           result_str)
