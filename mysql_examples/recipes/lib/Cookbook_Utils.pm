...
# element is a reference to a hash keyed by name of column in
...
my ($dbh, $db_name, $tbl_name) = @_;
  ...
  my $stmt = "SELECT * FROM INFORMATION_SCHEMA.COLUMNS
              WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ?";
...
# and nullable keys.  Return undef if no info available.
...
my $info = {};
  ...
  $info->{name} = $row[0];
  return undef unless $row[1] =~ /^(ENUM|SET)\((.*)\)$/i;
  $info->{type} = $1;
...
# common formats.  All allow the year to be 2 or 4 digits.  All allow
